...
const updateFirestore = async (
  ...
  await doc.ref
    ...
    .catch(() => {
      throw new functions.https.HttpsError('data-loss', 'ユーザーの編集に失敗しました', 'firebase');
    });

  return children;
};

const updateAlgolia = async (user: Data[number], child?: string): Promise<void> => {
  ...
  await index
    ...
    .catch(() => {
      throw new functions.https.HttpsError('data-loss', 'ユーザーの編集に失敗しました', 'algolia');
    });
};
