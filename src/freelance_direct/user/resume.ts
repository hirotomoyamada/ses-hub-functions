...
import { db, location, runtime, storage, converter } from "../../firebase";
...
const deleteFile = async (
  ...
  const name = `${key}.pdf`;
  const bucket = storage.bucket(functions.config().storage.resume as string);
  const path = bucket.file(name);
...
const updateFirestore = async (
  ...
  await doc.ref
    .set(
      {
        resume: key && url ? { key: key, url: url } : { key: null, url: null },
        updateAt: timestamp,
      },
      { merge: true }
    )
